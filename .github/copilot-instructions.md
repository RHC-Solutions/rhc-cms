# GitHub Copilot Instructions for RHC Solutions / admin_panel
...
It is **also the `RHC-Solutions/admin_panel` platform** — a reusable, embeddable CMS admin ("WordPress for the Node/Next/Postgres stack") that other sites pull in as a `vendor/admin-panel` git submodule. When editing shared code (`src/lib`, `src/components`, API routes), assume it runs **inside an arbitrary host**, not just this site — don't hardcode RHC paths/domains. See **[README.md](../README.md)**, **[docs/DESIGN_PACKS.md](../docs/DESIGN_PACKS.md)**, **[docs/DEPLOY_NEW_SITE.md](../docs/DEPLOY_NEW_SITE.md)**, and **[CLAUDE.md](../CLAUDE.md)** ("This repo is also a platform" section) for the authoritative conventions.

### Platform subsystems (newer than most of this file)
- **Distribution**: hosts embed via `npx github:RHC-Solutions/admin_panel init`; shared imports use the **`@adminpanel/*`** alias; routes are thin re-export wrappers generated by `scripts/install-into-site.mjs`; auth is `adminAuthGate` in the host `middleware.ts`. CLI: `bin/admin-panel.mjs` (`init`/`update`/`apply-pack`).
