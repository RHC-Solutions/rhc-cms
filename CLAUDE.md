...
This repo (`RHC-Solutions/admin_panel`) is the **embeddable CMS admin only** — `src/app/{admin,api,pack-preview}`, the shared `src/lib`/`src/components`, the installer, and the design-pack/provisioning/OODA subsystems. It is **not** a full website: there are **no** public pages, no `cms-data/`, no `src/app/robots.ts` / `sitemap.ts` / `(public)/*` here. Hosts pull this in as a `vendor/admin-panel` git submodule. "WordPress for the Node/Next/Postgres stack." When you edit shared code, assume it runs **inside an arbitrary host**, not just one site — don't hardcode RHC paths/domains.
...
- **Distribution.** Hosts embed via `npx github:RHC-Solutions/admin_panel init`; imports use the `@adminpanel/*` alias (→ `src/*`); routes are thin re-export wrappers generated by `scripts/install-into-site.mjs`; auth is `adminAuthGate` composed into the host's `middleware.ts`. CLI: `bin/admin-panel.mjs` (`init`/`update`/`apply-pack`, `--static-site`). Don't hardcode RHC paths/domains into shared lib code. Full picture: [README.md](./README.md), [INSTALL.md](./INSTALL.md).
